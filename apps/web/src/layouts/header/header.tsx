    ...
    isMounted && (
      ...
      >
        <div className="relative flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full border border-gray-100 bg-accent text-brand shadow-main transition-all hover:-translate-y-0.5 hover:shadow-large focus:-translate-y-0.5 focus:shadow-large focus:outline-none dark:border-gray-700 dark:bg-light-dark dark:text-white sm:h-12 sm:w-12">
          ...
          <BellIcon className="h-auto w-3 sm:w-auto" />
...
function HeaderRightArea() {
  ...
  return (
    <div className="relative order-last flex shrink-0 items-center gap-4 sm:gap-6 lg:gap-8">
      {account.isConnected && <NotificationButton />}
      <WalletConnect />
    </div>
  );
}
    ...
    <nav
      className={cn(
        ...
        isMounted && windowScroll.y > 17
          ? 'bg-accent/80 shadow-card dark:bg-dark/80'
          : '',
        className,
      )}
    >
  ...
  return (
    ...
    >
      <div className="flex h-full items-center justify-between px-4 sm:px-6 lg:px-8 3xl:px-10">
        ...
        <div className="flex items-center gap-4 rounded-full px-10 py-2">
          <LogoIcon />

          <h1 className="font-kaleko text-xl font-semibold tracking-wider text-gray-200">
            Crowd VC
          </h1>
        </div>
        <HeaderRightArea />
